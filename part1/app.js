...
app.get('/api/walkrequests/open', async (req, res) => {
  try {
    const [openRequests] = await db.execute(`SELECT r.request_id, d.name, r.requested_time, r.duration_minutes, r.location, status) SELECT dog_id, '2025-06-10 08:00:00', 30, 'Parklands', 1 FROM Dog
s WHERE dog_id = (SELECT dog_id FROM Dogs WHERE name = 'Max');`);
