...
await db.execute(`INSERT INTO Dogs (owner_id, name, size) SELECT user_id, 'Bella', 'small' FROM Users WHERE user_id = (SELECT user_id FROM Users WHERE username = 'carol1
23')`);
...
app.get('/api/walkers/summary', async (req, res) => {
  try {
    const [walkers] = await db.execute(`
        ...
        (SELECT COUNT(rating)
        FROM WalkRatings w
        WHERE w.walker_id = o.user_id) AS total_ratings,
        (SELECT AVG(rating)
        FROM WalkRatings w
        WHERE w.walker_id = o.user_id) AS average_rating,
        (SELECT COUNT(status)
        FROM WalkRequests r
        JOIN WalkRatings w ON r.request_id = w.request_id
        WHERE w.walker_id = o.user_id AND status = 'completed') AS completed_walks
        FROM Users o
        WHERE o.role = 'walker'
        `);
    res.json(walkers);
